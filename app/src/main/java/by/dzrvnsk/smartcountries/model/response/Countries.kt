package by.dzrvnsk.smartcountries.model.response

import com.google.gson.annotations.SerializedName

data class Countries(

	@field:SerializedName("Countries")
	val countries: ArrayList<Country>
)

data class Afr(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class TRY(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Sag(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Nbl(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Mfe(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Tha(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Mlg(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class CUP(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Swe(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class SZL(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Vie(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Por(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class AZN(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class YER(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Mey(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Khm(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Heb(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class SDG(

	@field:SerializedName("name")
	val name: String
)

data class Nor(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class FJD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Smi(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class MWK(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class SGD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Cat(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Ind(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Prs(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Kat(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Mri(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class RUB(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class PYG(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Pau(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Spa(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class CAD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class COP(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class NGN(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class MZN(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Flags(

	@field:SerializedName("svg")
	val svg: String,

	@field:SerializedName("png")
	val png: String
)

data class Mlt(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Nau(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Nya(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class UGX(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class GHS(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class KMF(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class MAD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Tur(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Tuk(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class BZD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class NIO(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Nld(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class HUF(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class VUV(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class BRL(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Lin(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class JOD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class GIP(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Gle(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Lit(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Nde(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class BGN(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Idd(

	@field:SerializedName("suffixes")
	val suffixes: List<String>,

	@field:SerializedName("root")
	val root: String
)

data class Hin(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class MUR(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class EUR(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class SBD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Ton(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class USD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class KWD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Fas(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class STN(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class ZMW(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Eng(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String,

	@field:SerializedName("f")
	val F: String,

	@field:SerializedName("m")
	val M: String
)

data class AMD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class MVR(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class UZS(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class ZWL(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Som(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Aym(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class RWF(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class HKD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Tam(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Run(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Sqi(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class BAM(

	@field:SerializedName("name")
	val name: String
)

data class PostalCode(

	@field:SerializedName("regex")
	val regex: String,

	@field:SerializedName("format")
	val format: String
)

data class XOF(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class IQD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class AWG(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Car(

	@field:SerializedName("side")
	val side: String,

	@field:SerializedName("signs")
	val signs: List<String>
)

data class Fil(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class BND(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Kaz(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class ILS(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class XAF(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Nob(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Slk(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class BIF(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class SEK(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Est(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Ces(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class THB(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Dan(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class JPY(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class BDT(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class QAR(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class AED(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class KZT(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Ell(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class PGK(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class SAR(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Smo(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Div(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Kon(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class BWP(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class TJS(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Fao(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class SOS(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class KPW(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Que(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class FOK(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class WST(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class PHP(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class DZD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class KRW(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class SHP(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Bar(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Pov(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Bjz(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class CNY(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Zib(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class TOP(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class MDL(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Fij(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Zdj(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Pap(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class LKR(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class SRD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class NOK(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class SCR(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Crs(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class HRK(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Nrf(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Hye(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Languages(

	@field:SerializedName("eng")
	val eng: String,

	@field:SerializedName("urd")
	val urd: String,

	@field:SerializedName("fil")
	val fil: String,

	@field:SerializedName("rus")
	val rus: String,

	@field:SerializedName("kir")
	val kir: String,

	@field:SerializedName("jpn")
	val jpn: String,

	@field:SerializedName("tam")
	val tam: String,

	@field:SerializedName("sin")
	val sin: String,

	@field:SerializedName("ara")
	val ara: String,

	@field:SerializedName("arc")
	val arc: String,

	@field:SerializedName("ckb")
	val ckb: String,

	@field:SerializedName("cat")
	val cat: String,

	@field:SerializedName("ukr")
	val ukr: String,

	@field:SerializedName("ton")
	val ton: String,

	@field:SerializedName("nor")
	val nor: String,

	@field:SerializedName("kaz")
	val kaz: String,

	@field:SerializedName("swa")
	val swa: String,

	@field:SerializedName("dan")
	val dan: String,

	@field:SerializedName("fao")
	val fao: String,

	@field:SerializedName("spa")
	val spa: String,

	@field:SerializedName("ben")
	val ben: String,

	@field:SerializedName("fra")
	val fra: String,

	@field:SerializedName("sag")
	val sag: String,

	@field:SerializedName("prs")
	val prs: String,

	@field:SerializedName("tuk")
	val tuk: String,

	@field:SerializedName("pus")
	val pus: String,

	@field:SerializedName("bar")
	val bar: String,

	@field:SerializedName("hye")
	val hye: String,

	@field:SerializedName("mlg")
	val mlg: String,

	@field:SerializedName("hrv")
	val hrv: String,

	@field:SerializedName("nya")
	val nya: String,

	@field:SerializedName("vie")
	val vie: String,

	@field:SerializedName("por")
	val por: String,

	@field:SerializedName("tur")
	val tur: String,

	@field:SerializedName("ell")
	val ell: String,

	@field:SerializedName("nld")
	val nld: String,

	@field:SerializedName("lit")
	val lit: String,

	@field:SerializedName("lav")
	val lav: String,

	@field:SerializedName("pov")
	val pov: String,

	@field:SerializedName("tha")
	val tha: String,

	@field:SerializedName("lin")
	val lin: String,

	@field:SerializedName("kon")
	val kon: String,

	@field:SerializedName("deu")
	val deu: String,

	@field:SerializedName("zdj")
	val zdj: String,

	@field:SerializedName("kor")
	val kor: String,

	@field:SerializedName("fas")
	val fas: String,

	@field:SerializedName("sqi")
	val sqi: String,

	@field:SerializedName("kat")
	val kat: String,

	@field:SerializedName("ind")
	val ind: String,

	@field:SerializedName("mon")
	val mon: String,

	@field:SerializedName("jam")
	val jam: String,

	@field:SerializedName("cha")
	val cha: String,

	@field:SerializedName("cal")
	val cal: String,

	@field:SerializedName("isl")
	val isl: String,

	@field:SerializedName("swe")
	val swe: String,

	@field:SerializedName("fin")
	val fin: String,

	@field:SerializedName("mey")
	val mey: String,

	@field:SerializedName("ber")
	val ber: String,

	@field:SerializedName("mya")
	val mya: String,

	@field:SerializedName("mfe")
	val mfe: String,

	@field:SerializedName("amh")
	val amh: String,

	@field:SerializedName("grn")
	val grn: String,

	@field:SerializedName("zho")
	val zho: String,

	@field:SerializedName("hun")
	val hun: String,

	@field:SerializedName("pau")
	val pau: String,

	@field:SerializedName("gil")
	val gil: String,

	@field:SerializedName("cnr")
	val cnr: String,

	@field:SerializedName("bel")
	val bel: String,

	@field:SerializedName("glv")
	val glv: String,

	@field:SerializedName("slv")
	val slv: String,

	@field:SerializedName("nrf")
	val nrf: String,

	@field:SerializedName("srp")
	val srp: String,

	@field:SerializedName("msa")
	val msa: String,

	@field:SerializedName("niu")
	val niu: String,

	@field:SerializedName("slk")
	val slk: String,

	@field:SerializedName("nep")
	val nep: String,

	@field:SerializedName("div")
	val div: String,

	@field:SerializedName("ita")
	val ita: String,

	@field:SerializedName("mlt")
	val mlt: String,

	@field:SerializedName("pol")
	val pol: String,

	@field:SerializedName("hif")
	val hif: String,

	@field:SerializedName("fij")
	val fij: String,

	@field:SerializedName("aze")
	val aze: String,

	@field:SerializedName("que")
	val que: String,

	@field:SerializedName("aym")
	val aym: String,

	@field:SerializedName("sot")
	val sot: String,

	@field:SerializedName("pap")
	val pap: String,

	@field:SerializedName("sna")
	val sna: String,

	@field:SerializedName("ndc")
	val ndc: String,

	@field:SerializedName("khi")
	val khi: String,

	@field:SerializedName("nde")
	val nde: String,

	@field:SerializedName("tsn")
	val tsn: String,

	@field:SerializedName("toi")
	val toi: String,

	@field:SerializedName("tso")
	val tso: String,

	@field:SerializedName("kck")
	val kck: String,

	@field:SerializedName("zib")
	val zib: String,

	@field:SerializedName("bwg")
	val bwg: String,

	@field:SerializedName("ven")
	val ven: String,

	@field:SerializedName("xho")
	val xho: String,

	@field:SerializedName("tet")
	val tet: String,

	@field:SerializedName("zul")
	val zul: String,

	@field:SerializedName("ssw")
	val ssw: String,

	@field:SerializedName("nbl")
	val nbl: String,

	@field:SerializedName("nso")
	val nso: String,

	@field:SerializedName("afr")
	val afr: String,

	@field:SerializedName("ltz")
	val ltz: String,

	@field:SerializedName("bjz")
	val bjz: String,

	@field:SerializedName("est")
	val est: String,

	@field:SerializedName("nob")
	val nob: String,

	@field:SerializedName("nno")
	val nno: String,

	@field:SerializedName("smi")
	val smi: String,

	@field:SerializedName("som")
	val som: String,

	@field:SerializedName("bos")
	val bos: String,

	@field:SerializedName("bul")
	val bul: String,

	@field:SerializedName("gle")
	val gle: String,

	@field:SerializedName("mri")
	val mri: String,

	@field:SerializedName("nzs")
	val nzs: String,

	@field:SerializedName("hin")
	val hin: String,

	@field:SerializedName("dzo")
	val dzo: String,

	@field:SerializedName("kin")
	val kin: String,

	@field:SerializedName("gsw")
	val gsw: String,

	@field:SerializedName("roh")
	val roh: String,

	@field:SerializedName("tvl")
	val tvl: String,

	@field:SerializedName("lua")
	val lua: String,

	@field:SerializedName("uzb")
	val uzb: String,

	@field:SerializedName("tir")
	val tir: String,

	@field:SerializedName("hmo")
	val hmo: String,

	@field:SerializedName("tpi")
	val tpi: String,

	@field:SerializedName("mah")
	val mah: String,

	@field:SerializedName("ces")
	val ces: String,

	@field:SerializedName("smo")
	val smo: String,

	@field:SerializedName("tkl")
	val tkl: String,

	@field:SerializedName("pih")
	val pih: String,

	@field:SerializedName("run")
	val run: String,

	@field:SerializedName("nau")
	val nau: String,

	@field:SerializedName("bis")
	val bis: String,

	@field:SerializedName("lat")
	val lat: String,

	@field:SerializedName("kal")
	val kal: String,

	@field:SerializedName("heb")
	val heb: String,

	@field:SerializedName("crs")
	val crs: String,

	@field:SerializedName("rar")
	val rar: String,

	@field:SerializedName("khm")
	val khm: String,

	@field:SerializedName("kwn")
	val kwn: String,

	@field:SerializedName("her")
	val her: String,

	@field:SerializedName("loz")
	val loz: String,

	@field:SerializedName("ndo")
	val ndo: String,

	@field:SerializedName("hgm")
	val hgm: String,

	@field:SerializedName("lao")
	val lao: String,

	@field:SerializedName("nfr")
	val nfr: String,

	@field:SerializedName("tgk")
	val tgk: String,

	@field:SerializedName("hat")
	val hat: String
)

data class GMD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class XPF(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class SSP(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Slv(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class GYD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class ALL(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Nfr(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Pus(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Fra(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String,

	@field:SerializedName("f")
	val F: String,

	@field:SerializedName("m")
	val M: String
)

data class TND(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class PLN(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class DOP(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class LBP(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class CUC(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class UYU(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class MNT(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class BOB(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class UAH(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Bwg(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Amh(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Hgm(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Ukr(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class DJF(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Hif(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Hat(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class AUD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Tvl(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Nno(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class SLL(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class PAB(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Kir(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Mah(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class GGP(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class EGP(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class NPR(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Msa(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class ERN(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Rus(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Lat(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Ssw(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Nzs(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Ckb(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class OMR(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Sot(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Name(

	@field:SerializedName("nativeName")
	val nativeName: NativeName,

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Bel(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Gsw(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Tkl(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class DKK(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Bos(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class BMD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class JMD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Ltz(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Jpn(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Bul(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Rar(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Currencies(

	@field:SerializedName("USD")
	val uSD: USD,

	@field:SerializedName("PKR")
	val pKR: PKR,

	@field:SerializedName("PHP")
	val pHP: PHP,

	@field:SerializedName("KGS")
	val kGS: KGS,

	@field:SerializedName("JPY")
	val jPY: JPY,

	@field:SerializedName("LKR")
	val lKR: LKR,

	@field:SerializedName("IQD")
	val iQD: IQD,

	@field:SerializedName("EUR")
	val eUR: EUR,

	@field:SerializedName("QAR")
	val qAR: QAR,

	@field:SerializedName("UAH")
	val uAH: UAH,

	@field:SerializedName("TOP")
	val tOP: TOP,

	@field:SerializedName("SHP")
	val sHP: SHP,

	@field:SerializedName("KZT")
	val kZT: KZT,

	@field:SerializedName("UGX")
	val uGX: UGX,

	@field:SerializedName("FOK")
	val fOK: FOK,

	@field:SerializedName("DKK")
	val dKK: DKK,

	@field:SerializedName("CRC")
	val cRC: CRC,

	@field:SerializedName("BDT")
	val bDT: BDT,

	@field:SerializedName("ZMW")
	val zMW: ZMW,

	@field:SerializedName("XAF")
	val xAF: XAF,

	@field:SerializedName("AFN")
	val aFN: AFN,

	@field:SerializedName("AMD")
	val aMD: AMD,

	@field:SerializedName("MGA")
	val mGA: MGA,

	@field:SerializedName("HRK")
	val hRK: HRK,

	@field:SerializedName("MWK")
	val mWK: MWK,

	@field:SerializedName("TTD")
	val tTD: TTD,

	@field:SerializedName("XPF")
	val xPF: XPF,

	@field:SerializedName("VND")
	val vND: VND,

	@field:SerializedName("CVE")
	val cVE: CVE,

	@field:SerializedName("SRD")
	val sRD: SRD,

	@field:SerializedName("YER")
	val yER: YER,

	@field:SerializedName("GYD")
	val gYD: GYD,

	@field:SerializedName("PAB")
	val pAB: PAB,

	@field:SerializedName("XOF")
	val xOF: XOF,

	@field:SerializedName("AUD")
	val aUD: AUD,

	@field:SerializedName("THB")
	val tHB: THB,

	@field:SerializedName("BBD")
	val bBD: BBD,

	@field:SerializedName("KMF")
	val kMF: KMF,

	@field:SerializedName("AOA")
	val aOA: AOA,

	@field:SerializedName("KRW")
	val kRW: KRW,

	@field:SerializedName("NGN")
	val nGN: NGN,

	@field:SerializedName("XCD")
	val xCD: XCD,

	@field:SerializedName("IRR")
	val iRR: IRR,

	@field:SerializedName("GIP")
	val gIP: GIP,

	@field:SerializedName("BRL")
	val bRL: BRL,

	@field:SerializedName("ALL")
	val aLL: ALL,

	@field:SerializedName("GEL")
	val gEL: GEL,

	@field:SerializedName("IDR")
	val iDR: IDR,

	@field:SerializedName("MNT")
	val mNT: MNT,

	@field:SerializedName("JMD")
	val jMD: JMD,

	@field:SerializedName("ISK")
	val iSK: ISK,

	@field:SerializedName("MAD")
	val mAD: MAD,

	@field:SerializedName("MRU")
	val mRU: MRU,

	@field:SerializedName("DZD")
	val dZD: DZD,

	@field:SerializedName("TND")
	val tND: TND,

	@field:SerializedName("LRD")
	val lRD: LRD,

	@field:SerializedName("MMK")
	val mMK: MMK,

	@field:SerializedName("MUR")
	val mUR: MUR,

	@field:SerializedName("ETB")
	val eTB: ETB,

	@field:SerializedName("BMD")
	val bMD: BMD,

	@field:SerializedName("SAR")
	val sAR: SAR,

	@field:SerializedName("ARS")
	val aRS: ARS,

	@field:SerializedName("TMT")
	val tMT: TMT,

	@field:SerializedName("GBP")
	val gBP: GBP,

	@field:SerializedName("HKD")
	val hKD: HKD,

	@field:SerializedName("SDG")
	val sDG: SDG,

	@field:SerializedName("COP")
	val cOP: COP,

	@field:SerializedName("HUF")
	val hUF: HUF,

	@field:SerializedName("SEK")
	val sEK: SEK,

	@field:SerializedName("FKP")
	val fKP: FKP,

	@field:SerializedName("NZD")
	val nZD: NZD,

	@field:SerializedName("CHF")
	val cHF: CHF,

	@field:SerializedName("KID")
	val kID: KID,

	@field:SerializedName("CUC")
	val cUC: CUC,

	@field:SerializedName("CUP")
	val cUP: CUP,

	@field:SerializedName("BYN")
	val bYN: BYN,

	@field:SerializedName("IMP")
	val iMP: IMP,

	@field:SerializedName("JEP")
	val jEP: JEP,

	@field:SerializedName("SGD")
	val sGD: SGD,

	@field:SerializedName("OMR")
	val oMR: OMR,

	@field:SerializedName("LBP")
	val lBP: LBP,

	@field:SerializedName("TWD")
	val tWD: TWD,

	@field:SerializedName("NPR")
	val nPR: NPR,

	@field:SerializedName("MVR")
	val mVR: MVR,

	@field:SerializedName("HNL")
	val hNL: HNL,

	@field:SerializedName("GHS")
	val gHS: GHS,

	@field:SerializedName("NOK")
	val nOK: NOK,

	@field:SerializedName("NIO")
	val nIO: NIO,

	@field:SerializedName("RUB")
	val rUB: RUB,

	@field:SerializedName("ILS")
	val iLS: ILS,

	@field:SerializedName("JOD")
	val jOD: JOD,

	@field:SerializedName("EGP")
	val eGP: EGP,

	@field:SerializedName("SLL")
	val sLL: SLL,

	@field:SerializedName("PLN")
	val pLN: PLN,

	@field:SerializedName("FJD")
	val fJD: FJD,

	@field:SerializedName("AZN")
	val aZN: AZN,

	@field:SerializedName("STN")
	val sTN: STN,

	@field:SerializedName("DJF")
	val dJF: DJF,

	@field:SerializedName("ANG")
	val aNG: ANG,

	@field:SerializedName("PEN")
	val pEN: PEN,

	@field:SerializedName("LSL")
	val lSL: LSL,

	@field:SerializedName("ZAR")
	val zAR: ZAR,

	@field:SerializedName("GTQ")
	val gTQ: GTQ,

	@field:SerializedName("CLP")
	val cLP: CLP,

	@field:SerializedName("MZN")
	val mZN: MZN,

	@field:SerializedName("ZWL")
	val zWL: ZWL,

	@field:SerializedName("GNF")
	val gNF: GNF,

	@field:SerializedName("PYG")
	val pYG: PYG,

	@field:SerializedName("BZD")
	val bZD: BZD,

	@field:SerializedName("GMD")
	val gMD: GMD,

	@field:SerializedName("SOS")
	val sOS: SOS,

	@field:SerializedName("BAM")
	val bAM: BAM,

	@field:SerializedName("VES")
	val vES: VES,

	@field:SerializedName("KES")
	val kES: KES,

	@field:SerializedName("SZL")
	val sZL: SZL,

	@field:SerializedName("BGN")
	val bGN: BGN,

	@field:SerializedName("MOP")
	val mOP: MOP,

	@field:SerializedName("INR")
	val iNR: INR,

	@field:SerializedName("BWP")
	val bWP: BWP,

	@field:SerializedName("BTN")
	val bTN: BTN,

	@field:SerializedName("RWF")
	val rWF: RWF,

	@field:SerializedName("LYD")
	val lYD: LYD,

	@field:SerializedName("CAD")
	val cAD: CAD,

	@field:SerializedName("TVD")
	val tVD: TVD,

	@field:SerializedName("CDF")
	val cDF: CDF,

	@field:SerializedName("UYU")
	val uYU: UYU,

	@field:SerializedName("UZS")
	val uZS: UZS,

	@field:SerializedName("MDL")
	val mDL: MDL,

	@field:SerializedName("ERN")
	val eRN: ERN,

	@field:SerializedName("PGK")
	val pGK: PGK,

	@field:SerializedName("TZS")
	val tZS: TZS,

	@field:SerializedName("SYP")
	val sYP: SYP,

	@field:SerializedName("CZK")
	val cZK: CZK,

	@field:SerializedName("AWG")
	val aWG: AWG,

	@field:SerializedName("SSP")
	val sSP: SSP,

	@field:SerializedName("KPW")
	val kPW: KPW,

	@field:SerializedName("DOP")
	val dOP: DOP,

	@field:SerializedName("BHD")
	val bHD: BHD,

	@field:SerializedName("BSD")
	val bSD: BSD,

	@field:SerializedName("BND")
	val bND: BND,

	@field:SerializedName("RSD")
	val rSD: RSD,

	@field:SerializedName("BIF")
	val bIF: BIF,

	@field:SerializedName("KWD")
	val kWD: KWD,

	@field:SerializedName("TRY")
	val tRY: TRY,

	@field:SerializedName("SBD")
	val sBD: SBD,

	@field:SerializedName("VUV")
	val vUV: VUV,

	@field:SerializedName("CNY")
	val cNY: CNY,

	@field:SerializedName("AED")
	val aED: AED,

	@field:SerializedName("BOB")
	val bOB: BOB,

	@field:SerializedName("SCR")
	val sCR: SCR,

	@field:SerializedName("CKD")
	val cKD: CKD,

	@field:SerializedName("KHR")
	val kHR: KHR,

	@field:SerializedName("KYD")
	val kYD: KYD,

	@field:SerializedName("NAD")
	val nAD: NAD,

	@field:SerializedName("LAK")
	val lAK: LAK,

	@field:SerializedName("GGP")
	val gGP: GGP,

	@field:SerializedName("WST")
	val wST: WST,

	@field:SerializedName("MXN")
	val mXN: MXN,

	@field:SerializedName("TJS")
	val tJS: TJS,

	@field:SerializedName("HTG")
	val hTG: HTG,

	@field:SerializedName("MYR")
	val mYR: MYR
)

data class AFN(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Bis(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class RSD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class LAK(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class KES(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Isl(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Zho(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class INR(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class HNL(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class GBP(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class ANG(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Niu(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class CDF(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class BTN(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class CZK(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Tgk(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Pih(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class MGA(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class LYD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class BBD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Hrv(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class MXN(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Sna(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Cha(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class BHD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class VND(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Hun(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class GEL(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class LRD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Jam(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class KID(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Mya(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Hmo(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Dzo(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Ven(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Loz(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class NAD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class IMP(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Urd(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Ara(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Tso(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class CRC(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Ben(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class GTQ(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class TTD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class MMK(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Gil(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Per(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Translations(

	@field:SerializedName("hun")
	val hun: Hun,

	@field:SerializedName("swe")
	val swe: Swe,

	@field:SerializedName("zho")
	val zho: Zho,

	@field:SerializedName("spa")
	val spa: Spa,

	@field:SerializedName("urd")
	val urd: Urd,

	@field:SerializedName("est")
	val est: Est,

	@field:SerializedName("nld")
	val nld: Nld,

	@field:SerializedName("jpn")
	val jpn: Jpn,

	@field:SerializedName("fin")
	val fin: Fin,

	@field:SerializedName("pol")
	val pol: Pol,

	@field:SerializedName("hrv")
	val hrv: Hrv,

	@field:SerializedName("kor")
	val kor: Kor,

	@field:SerializedName("ces")
	val ces: Ces,

	@field:SerializedName("ara")
	val ara: Ara,

	@field:SerializedName("rus")
	val rus: Rus,

	@field:SerializedName("por")
	val por: Por,

	@field:SerializedName("slk")
	val slk: Slk,

	@field:SerializedName("fra")
	val fra: Fra,

	@field:SerializedName("deu")
	val deu: Deu,

	@field:SerializedName("cym")
	val cym: Cym,

	@field:SerializedName("ita")
	val ita: Ita,

	@field:SerializedName("per")
	val per: Per
)

data class ZAR(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class TWD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Ndo(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Pol(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class BYN(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class LSL(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Maps(

	@field:SerializedName("openStreetMaps")
	val openStreetMaps: String,

	@field:SerializedName("googleMaps")
	val googleMaps: String
)

data class MRU(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Arc(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Gini(

	@field:SerializedName("2018")
	val jsonMember2018: Double,

	@field:SerializedName("2019")
	val jsonMember2019: Double,

	@field:SerializedName("2013")
	val jsonMember2013: Double,

	@field:SerializedName("2016")
	val jsonMember2016: Double,

	@field:SerializedName("2012")
	val jsonMember2012: Double,

	@field:SerializedName("2015")
	val jsonMember2015: Double,

	@field:SerializedName("2008")
	val jsonMember2008: Double,

	@field:SerializedName("1992")
	val jsonMember1992: Double,

	@field:SerializedName("1999")
	val jsonMember1999: Double,

	@field:SerializedName("2014")
	val jsonMember2014: Double,

	@field:SerializedName("1998")
	val jsonMember1998: Double,

	@field:SerializedName("2010")
	val jsonMember2010: Double,

	@field:SerializedName("2011")
	val jsonMember2011: Double,

	@field:SerializedName("2017")
	val jsonMember2017: Double,

	@field:SerializedName("2004")
	val jsonMember2004: Double,

	@field:SerializedName("2006")
	val jsonMember2006: Double,

	@field:SerializedName("2005")
	val jsonMember2005: Double,

	@field:SerializedName("2009")
	val jsonMember2009: Double,

	@field:SerializedName("2003")
	val jsonMember2003: Double
)

data class PEN(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Deu(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class ISK(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Lao(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class TZS(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Ita(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Zul(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Her(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Ber(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class XCD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Mon(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class NZD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Swa(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Lua(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class BSD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class ARS(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class IDR(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class CKD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class CVE(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Ndc(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Grn(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Sin(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class JEP(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class SYP(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class MOP(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Nso(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Srp(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class CHF(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Uzb(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class CLP(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Glv(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Country(

	@field:SerializedName("capital")
	val capital: List<String>,

	@field:SerializedName("flag")
	val flag: String,

	@field:SerializedName("independent")
	val independent: Boolean,

	@field:SerializedName("landlocked")
	val landlocked: Boolean,

	@field:SerializedName("flags")
	val flags: Flags,

	@field:SerializedName("capitalInfo")
	val capitalInfo: CapitalInfo,

	@field:SerializedName("ccn3")
	val ccn3: String,

	@field:SerializedName("coatOfArms")
	val coatOfArms: CoatOfArms,

	@field:SerializedName("demonyms")
	val demonyms: Demonyms,

	@field:SerializedName("car")
	val car: Car,

	@field:SerializedName("translations")
	val translations: Translations,

	@field:SerializedName("altSpellings")
	val altSpellings: List<String>,

	@field:SerializedName("area")
	val area: Double,

	@field:SerializedName("languages")
	val languages: Languages,

	@field:SerializedName("maps")
	val maps: Maps,

	@field:SerializedName("subregion")
	val subregion: String,

	@field:SerializedName("idd")
	val idd: Idd,

	@field:SerializedName("tld")
	val tld: List<String>,

	@field:SerializedName("unMember")
	val unMember: Boolean,

	@field:SerializedName("continents")
	val continents: List<String>,

	@field:SerializedName("population")
	val population: Int,

	@field:SerializedName("startOfWeek")
	val startOfWeek: String,

	@field:SerializedName("timezones")
	val timezones: List<String>,

	@field:SerializedName("name")
	val name: Name,

	@field:SerializedName("cca3")
	val cca3: String,

	@field:SerializedName("region")
	val region: String,

	@field:SerializedName("latlng")
	val latlng: List<Double>,

	@field:SerializedName("cca2")
	val cca2: String,

	@field:SerializedName("status")
	val status: String,

	@field:SerializedName("currencies")
	val currencies: Currencies,

	@field:SerializedName("borders")
	val borders: List<String>,

	@field:SerializedName("postalCode")
	val postalCode: PostalCode,

	@field:SerializedName("fifa")
	val fifa: String,

	@field:SerializedName("cioc")
	val cioc: String,

	@field:SerializedName("gini")
	val gini: Gini
)

data class Xho(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class TVD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class GNF(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class NativeName(

	@field:SerializedName("eng")
	val eng: Eng,

	@field:SerializedName("urd")
	val urd: Urd,

	@field:SerializedName("fil")
	val fil: Fil,

	@field:SerializedName("rus")
	val rus: Rus,

	@field:SerializedName("kir")
	val kir: Kir,

	@field:SerializedName("jpn")
	val jpn: Jpn,

	@field:SerializedName("tam")
	val tam: Tam,

	@field:SerializedName("sin")
	val sin: Sin,

	@field:SerializedName("ara")
	val ara: Ara,

	@field:SerializedName("arc")
	val arc: Arc,

	@field:SerializedName("ckb")
	val ckb: Ckb,

	@field:SerializedName("cat")
	val cat: Cat,

	@field:SerializedName("ukr")
	val ukr: Ukr,

	@field:SerializedName("ton")
	val ton: Ton,

	@field:SerializedName("nor")
	val nor: Nor,

	@field:SerializedName("kaz")
	val kaz: Kaz,

	@field:SerializedName("swa")
	val swa: Swa,

	@field:SerializedName("dan")
	val dan: Dan,

	@field:SerializedName("fao")
	val fao: Fao,

	@field:SerializedName("spa")
	val spa: Spa,

	@field:SerializedName("ben")
	val ben: Ben,

	@field:SerializedName("fra")
	val fra: Fra,

	@field:SerializedName("sag")
	val sag: Sag,

	@field:SerializedName("prs")
	val prs: Prs,

	@field:SerializedName("tuk")
	val tuk: Tuk,

	@field:SerializedName("pus")
	val pus: Pus,

	@field:SerializedName("bar")
	val bar: Bar,

	@field:SerializedName("hye")
	val hye: Hye,

	@field:SerializedName("mlg")
	val mlg: Mlg,

	@field:SerializedName("hrv")
	val hrv: Hrv,

	@field:SerializedName("nya")
	val nya: Nya,

	@field:SerializedName("vie")
	val vie: Vie,

	@field:SerializedName("por")
	val por: Por,

	@field:SerializedName("tur")
	val tur: Tur,

	@field:SerializedName("ell")
	val ell: Ell,

	@field:SerializedName("nld")
	val nld: Nld,

	@field:SerializedName("lit")
	val lit: Lit,

	@field:SerializedName("lav")
	val lav: Lav,

	@field:SerializedName("pov")
	val pov: Pov,

	@field:SerializedName("tha")
	val tha: Tha,

	@field:SerializedName("lin")
	val lin: Lin,

	@field:SerializedName("kon")
	val kon: Kon,

	@field:SerializedName("deu")
	val deu: Deu,

	@field:SerializedName("zdj")
	val zdj: Zdj,

	@field:SerializedName("kor")
	val kor: Kor,

	@field:SerializedName("fas")
	val fas: Fas,

	@field:SerializedName("sqi")
	val sqi: Sqi,

	@field:SerializedName("kat")
	val kat: Kat,

	@field:SerializedName("ind")
	val ind: Ind,

	@field:SerializedName("mon")
	val mon: Mon,

	@field:SerializedName("jam")
	val jam: Jam,

	@field:SerializedName("cha")
	val cha: Cha,

	@field:SerializedName("cal")
	val cal: Cal,

	@field:SerializedName("isl")
	val isl: Isl,

	@field:SerializedName("swe")
	val swe: Swe,

	@field:SerializedName("fin")
	val fin: Fin,

	@field:SerializedName("mey")
	val mey: Mey,

	@field:SerializedName("ber")
	val ber: Ber,

	@field:SerializedName("mya")
	val mya: Mya,

	@field:SerializedName("mfe")
	val mfe: Mfe,

	@field:SerializedName("amh")
	val amh: Amh,

	@field:SerializedName("grn")
	val grn: Grn,

	@field:SerializedName("zho")
	val zho: Zho,

	@field:SerializedName("hun")
	val hun: Hun,

	@field:SerializedName("pau")
	val pau: Pau,

	@field:SerializedName("gil")
	val gil: Gil,

	@field:SerializedName("cnr")
	val cnr: Cnr,

	@field:SerializedName("bel")
	val bel: Bel,

	@field:SerializedName("glv")
	val glv: Glv,

	@field:SerializedName("slv")
	val slv: Slv,

	@field:SerializedName("nrf")
	val nrf: Nrf,

	@field:SerializedName("srp")
	val srp: Srp,

	@field:SerializedName("msa")
	val msa: Msa,

	@field:SerializedName("niu")
	val niu: Niu,

	@field:SerializedName("slk")
	val slk: Slk,

	@field:SerializedName("nep")
	val nep: Nep,

	@field:SerializedName("div")
	val div: Div,

	@field:SerializedName("ita")
	val ita: Ita,

	@field:SerializedName("mlt")
	val mlt: Mlt,

	@field:SerializedName("pol")
	val pol: Pol,

	@field:SerializedName("hif")
	val hif: Hif,

	@field:SerializedName("fij")
	val fij: Fij,

	@field:SerializedName("aze")
	val aze: Aze,

	@field:SerializedName("que")
	val que: Que,

	@field:SerializedName("aym")
	val aym: Aym,

	@field:SerializedName("sot")
	val sot: Sot,

	@field:SerializedName("pap")
	val pap: Pap,

	@field:SerializedName("sna")
	val sna: Sna,

	@field:SerializedName("ndc")
	val ndc: Ndc,

	@field:SerializedName("khi")
	val khi: Khi,

	@field:SerializedName("nde")
	val nde: Nde,

	@field:SerializedName("tsn")
	val tsn: Tsn,

	@field:SerializedName("toi")
	val toi: Toi,

	@field:SerializedName("tso")
	val tso: Tso,

	@field:SerializedName("kck")
	val kck: Kck,

	@field:SerializedName("zib")
	val zib: Zib,

	@field:SerializedName("bwg")
	val bwg: Bwg,

	@field:SerializedName("ven")
	val ven: Ven,

	@field:SerializedName("xho")
	val xho: Xho,

	@field:SerializedName("tet")
	val tet: Tet,

	@field:SerializedName("zul")
	val zul: Zul,

	@field:SerializedName("ssw")
	val ssw: Ssw,

	@field:SerializedName("nbl")
	val nbl: Nbl,

	@field:SerializedName("nso")
	val nso: Nso,

	@field:SerializedName("afr")
	val afr: Afr,

	@field:SerializedName("ltz")
	val ltz: Ltz,

	@field:SerializedName("bjz")
	val bjz: Bjz,

	@field:SerializedName("est")
	val est: Est,

	@field:SerializedName("nob")
	val nob: Nob,

	@field:SerializedName("nno")
	val nno: Nno,

	@field:SerializedName("smi")
	val smi: Smi,

	@field:SerializedName("som")
	val som: Som,

	@field:SerializedName("bos")
	val bos: Bos,

	@field:SerializedName("bul")
	val bul: Bul,

	@field:SerializedName("gle")
	val gle: Gle,

	@field:SerializedName("mri")
	val mri: Mri,

	@field:SerializedName("nzs")
	val nzs: Nzs,

	@field:SerializedName("hin")
	val hin: Hin,

	@field:SerializedName("dzo")
	val dzo: Dzo,

	@field:SerializedName("kin")
	val kin: Kin,

	@field:SerializedName("gsw")
	val gsw: Gsw,

	@field:SerializedName("roh")
	val roh: Roh,

	@field:SerializedName("tvl")
	val tvl: Tvl,

	@field:SerializedName("lua")
	val lua: Lua,

	@field:SerializedName("uzb")
	val uzb: Uzb,

	@field:SerializedName("tir")
	val tir: Tir,

	@field:SerializedName("hmo")
	val hmo: Hmo,

	@field:SerializedName("tpi")
	val tpi: Tpi,

	@field:SerializedName("mah")
	val mah: Mah,

	@field:SerializedName("ces")
	val ces: Ces,

	@field:SerializedName("smo")
	val smo: Smo,

	@field:SerializedName("tkl")
	val tkl: Tkl,

	@field:SerializedName("pih")
	val pih: Pih,

	@field:SerializedName("run")
	val run: Run,

	@field:SerializedName("nau")
	val nau: Nau,

	@field:SerializedName("bis")
	val bis: Bis,

	@field:SerializedName("lat")
	val lat: Lat,

	@field:SerializedName("kal")
	val kal: Kal,

	@field:SerializedName("heb")
	val heb: Heb,

	@field:SerializedName("crs")
	val crs: Crs,

	@field:SerializedName("rar")
	val rar: Rar,

	@field:SerializedName("khm")
	val khm: Khm,

	@field:SerializedName("kwn")
	val kwn: Kwn,

	@field:SerializedName("her")
	val her: Her,

	@field:SerializedName("loz")
	val loz: Loz,

	@field:SerializedName("ndo")
	val ndo: Ndo,

	@field:SerializedName("hgm")
	val hgm: Hgm,

	@field:SerializedName("lao")
	val lao: Lao,

	@field:SerializedName("nfr")
	val nfr: Nfr,

	@field:SerializedName("tgk")
	val tgk: Tgk,

	@field:SerializedName("hat")
	val hat: Hat
)

data class Toi(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Nep(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Tsn(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class IRR(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class KYD(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class HTG(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Fin(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Kin(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class KHR(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class AOA(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class MYR(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Khi(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Kor(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class PKR(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class FKP(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class TMT(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Roh(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class CapitalInfo(

	@field:SerializedName("latlng")
	val latlng: List<Double>
)

data class Cnr(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Tir(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Tpi(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Tet(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class KGS(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class ETB(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)

data class Aze(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Kck(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class CoatOfArms(

	@field:SerializedName("svg")
	val svg: String,

	@field:SerializedName("png")
	val png: String
)

data class Kal(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Demonyms(

	@field:SerializedName("eng")
	val eng: Eng,

	@field:SerializedName("fra")
	val fra: Fra
)

data class Kwn(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Lav(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Cal(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class Cym(

	@field:SerializedName("common")
	val common: String,

	@field:SerializedName("official")
	val official: String
)

data class VES(

	@field:SerializedName("symbol")
	val symbol: String,

	@field:SerializedName("name")
	val name: String
)
